...
class BBPModel:
    ...
    def __init__(self, x_flow, x_time, x_beat, x_sv, y_BP,
        ...
        self.x_sv_test = np.tile(x_sv[test_ind].reshape(-1,1), self.size)
    ...
    def model_train(self, Ew_model=2, physics_weight=1, batch=16, epochs=64):
        ...
        windk_model, _, unc_param = CNNTransformerNet(parameter_type='bbp').nn_model()
        opt = tf.keras.optimizers.Adam(clipnorm=10)
        ...
        for e in range(epochs):
            ...
            for b in range(len(train_batches)):
                ...
                with (tf.GradientTape() as tape):
                    ...
                    Z = tf.tile(tf.reshape(Z, (-1,1)), [1, self.size])
