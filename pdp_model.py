...
class PDPModel:
    ...
    def model_train(self, Ew_model=2, physics_weight=1, batch=16, epochs=64):
        ...
        # Constants
        pi, epsilon, thr, size = tf.constant(np.pi), tf.constant(1e-6), np.inf, 64
        ...
        for e in range(epochs):
            ...
            for b in range(len(train_batches)):
                ...
                with (tf.GradientTape() as tape):
                    ...
                    A_max = tf.tile(tf.expand_dims(tf.reshape(a_max, [-1]), axis=1), [1, size])
                    P0 = tf.tile(tf.expand_dims(tf.reshape(p0, [-1]), axis=1), [1, size])
                    P1 = tf.tile(tf.expand_dims(tf.reshape(p1, [-1]), axis=1), [1, size])
                    aorta_len = tf.tile(tf.expand_dims(tf.reshape(aorta_Len, [-1]), axis=1), [1, size])
