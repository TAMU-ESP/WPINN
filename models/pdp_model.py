...
class PDPModel:
    ...
    def model_train(self, Ew_model=2, physics_weight=1, batch=16, epochs=64):
        ...
        windk_model, params, unc_param = CNNTransformerNet(parameter_type='pdp').nn_model()
        ...
        for e in range(epochs):
            ...
            for b in range(len(train_batches)):
                ...
                with (tf.GradientTape() as tape):
                    ...
                    # Params
                    a_max = tf.tanh(params[0]) + 5
                    p0 = 40 * tf.tanh(params[1]) + 40
                    p1 = 20 * tf.tanh(params[2]) + 40
                    aorta_Len = 15 * tf.tanh(params[3]) + 65

                    A_max = tf.tile(tf.expand_dims(tf.reshape(a_max, [-1]), axis=1), [1,  self.size])
                    P0 = tf.tile(tf.expand_dims(tf.reshape(p0, [-1]), axis=1), [1, self.size])
                    P1 = tf.tile(tf.expand_dims(tf.reshape(p1, [-1]), axis=1), [1,  self.size])
                    aorta_len = tf.tile(tf.expand_dims(tf.reshape(aorta_Len, [-1]), axis=1), [1,  self.size])

                    C_prime = tf.math.divide(A_max, tf.math.multiply(pi, P1)) / (
                            1 + tf.math.pow(tf.math.divide(yp_BP_d - P0, P1 + epsilon), 2))
                    C = aorta_len * C_prime
                    C = tf.where(tf.math.is_nan(C), tf.fill(tf.shape(C), 5.0), C)

                    A = A_max * (0.5 + (1 / pi) * tf.math.atan((yp_BP_d - P0) / (P1 + epsilon)))
                    Z = (3 / 4000) * tf.math.sqrt(1060.0 / (tf.math.multiply(A, C_prime) + epsilon))
                    Z = tf.where(tf.math.is_nan(Z), tf.fill(tf.shape(Z), 0.2), Z)
                ...
                opt.apply_gradients(zip(gradients, windk_model.trainable_weights + [params, unc_param]))
