...
class BBPModel:
    ...
    def __init__(self, x_flow_train, x_time_train, x_beat_train, x_sv_train, y_BP_train,
               x_flow_val, x_time_val, x_beat_val, x_sv_val, y_BP_val,
               x_flow_test, x_time_test, x_beat_test, x_sv_test, y_BP_test):
        self.x_flow_train = x_flow_train
        self.x_time_train = x_time_train
        self.x_beat_train = x_beat_train
        self.x_sv_train = np.tile(x_sv_train.reshape(-1,1), 64)
        self.y_BP_train = y_BP_train
        self.x_flow_val = x_flow_val
        self.x_time_val = x_time_val
        self.x_beat_val = x_beat_val
        self.x_sv_val = np.tile(x_sv_val.reshape(-1,1), 64)
        self.y_BP_val = y_BP_val
        self.x_flow_test = x_flow_test
        self.x_time_test = x_time_test
        self.x_beat_test = x_beat_test
        self.x_sv_test = np.tile(x_sv_test.reshape(-1,1), 64)
    ...
    def preprocess(self):
        ...
        m_sv, s_sv = np.mean(self.x_sv_train), np.std(self.x_sv_train)
        ...
        x_sv_train, x_sv_val, x_sv_test = ((self.x_sv_train - m_sv) / s_sv,
                                           (self.x_sv_val - m_sv) / s_sv,
                                           (self.x_sv_test - m_sv) / s_sv)


        x_wave_train, y_train = (np.dstack([x_flow_train, x_time_train, x_beat_train, x_sv_train]),
                                 np.dstack([y_BP_train]))
        x_wave_val, y_val = np.dstack([x_flow_val, x_time_val, x_beat_val, x_sv_val]), np.dstack([y_BP_val])
        x_wave_test, y_test = np.dstack([x_flow_test, x_time_test, x_beat_test, x_sv_test]), np.dstack([y_BP_test])

        x_wave_val_test = np.concatenate([x_wave_val, x_wave_test], axis=0)
        return (m_flow, s_flow, m_time, s_time, m_BP, s_BP, m_sv, s_sv,
    ...
    def model_train(self, Ew_model=2, physics_weight=1, batch=16, epochs=64):
        ...
        (m_flow, s_flow, m_time, s_time, m_BP, s_BP, m_sv, s_sv,
         x_wave_train, y_train,
         x_wave_val, y_val,
         x_wave_test, y_test,
         x_wave_val_test)= BBPModel.preprocess(self)

        windk_model, _, unc_param = CNNTransformerNet().nn_model(input_shape=(64, 4), parameter_type='bbp')
        ...
        for e in range(epochs):
            ...
            for b in range(len(train_batches)):
                ...
                with (tf.GradientTape() as tape):
                    ...
                    # Params
                    sv = wave_tensor[:, :, 3] * s_sv + m_sv
                    pp = tf.tile(tf.reshape((tf.math.reduce_max(yp_BP_d, axis=-1) - yp_BP_d[:,-1]), (-1,1)),
                                 [1, 64])
                    C = sv / (pp + epsilon)


                    Z = tf.math.reduce_max(dyp_BP_dt, axis=-1) / (tf.math.reduce_max(dI_flow_dt, axis=-1) + epsilon)
                    Z = tf.tile(tf.reshape(Z, (-1,1)), [1, 64])
                    R = tf.reshape((K.mean(yp_BP_d, axis=-1)) / (K.mean(flow, axis=-1) + epsilon),(-1, 1))

                    Z = tf.clip_by_value(Z, 1e-2, 0.2)
                    C = tf.clip_by_value(C, 1e-2, 5)
                    R = tf.clip_by_value(R, 1e-2, 100)
                ...
                optimizer_windk_model.apply_gradients(zip(gradients,
                                                          windk_model.trainable_weights + [unc_param]))
